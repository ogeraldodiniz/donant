...
export default function NgoDetail() {
  ...
  if (!ngo) return <div className="container py-12 text-center"><p className="text-xl font-bold">ONG não encontrada</p></div>;
  ...
  return (
    <div className="container py-6 space-y-6 max-w-2xl">
      <Link to="/ongs" className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground">
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>

      <div className="text-center">
        <div className="text-5xl mb-3">{ngoEmojis[ngoIndex % ngoEmojis.length]}</div>
        <h1 className="text-2xl font-black">{ngo.name}</h1>
        {isSelected && (
          <span className="inline-flex items-center gap-1 text-xs font-bold text-primary bg-primary/10 px-3 py-1 rounded-full mt-2">
      ...
      <DuoCard className="text-center bg-primary/5 border-primary/20">
        <p className="text-sm font-bold text-muted-foreground mb-1">Total recebido</p>
        <p className="text-3xl font-black text-primary">R$ {ngo.total_received.toLocaleString('pt-BR')}</p>
      </DuoCard>
      ...
      {donations.length > 0 && (
        <DuoCard>
          <h3 className="font-bold mb-3">💚 Últimas doações</h3>
          <div className="space-y-2">
            {donations.map(d => (
              <div key={d.id} className="flex justify-between items-center py-2 border-b last:border-0 border-border">
                <span className="text-sm text-muted-foreground">{new Date(d.donated_at).toLocaleDateString('pt-BR')}</span>
                <span className="text-sm font-bold text-primary">R$ {d.amount.toFixed(2)}</span>
              </div>
            ))}
          </div>
        </DuoCard>
